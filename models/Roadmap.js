...
roadmapSchema.methods.getUpdatedSkills = function() {
  const updatedSkills = [];
  this.levels.forEach(phase => {
    phase.tasks.forEach(task => {
      if (task.completed && task.completedAt) {
        const now = new Date();
        const diff = (now - new Date(task.completedAt)) / (1000 * 60);
        if (diff < 5) {
          updatedSkills.push(...phase.skills);
        }
      }
    });
  });
  return [...new Set(updatedSkills)];
};
